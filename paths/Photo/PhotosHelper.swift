...
public struct PhotosHelper {
    ...
    public static func getAlbum(withId localId: String, completion: @escaping (_ album: PHAssetCollection?) -> ()) {
        DispatchQueue.global(qos: .background).async {
            let collections = PHAssetCollection.fetchAssetCollections(withLocalIdentifiers: [localId], options: nil)
    ...
    public static func saveImage(image: UIImage, to album: PHAssetCollection, completion: ((_ success: Bool, _ error: Error?) -> ())? = nil) {
        DispatchQueue.global(qos: .background).async{
            PHPhotoLibrary.shared().performChanges({
                ...
                albumChangeRequest?.addAssets([_placeholder] as NSArray)
            }) { success, error in
    ...
    public static func removeAll(from album: PHAssetCollection, completion: ((_ success: Bool, _ error: Error?) -> ())? = nil) {
        ...
        DispatchQueue.global(qos: .background).async{
            PHPhotoLibrary.shared().performChanges({
                ...
                var assets : [PHAsset] = []
                fetchedAssets.enumerateObjects({ (asset, index, finish) in
                    assets.append(asset)
                })
    ...
    public static func save(assets: [PHAsset], to album: PHAssetCollection, completion: ((_ success: Bool, _ error: Error?) -> ())? = nil) {
        DispatchQueue.global(qos: .background).async{
            PHPhotoLibrary.shared().performChanges({
                let albumChangeRequest = PHAssetCollectionChangeRequest(for: album)
    ...
    public static func getAssetsFromAlbum(album: PHAssetCollection, fetchOptions: FetchOptions = FetchOptions(), completion: @escaping (_ result: AssetFetchResult<PHAsset>) -> ()) {
        DispatchQueue.global(qos: .background).async{
            ...
            let range = 0...(fetchOptions.count != 0 ? rangeLength : fetchedAssets.count)
            let indexes = IndexSet(integersIn: range)
            
            fetchedAssets.enumerateObjects(at: indexes, options: []) { asset, index, stop in
                //                guard let asset = asset else { return completion(.Error) }
                assets.append(asset)
            }
